...
namespace Alyx.Jobs
{
    class Crow : Script
    {
        ...
        public static int JobPayment = 164;
        ...
        public static void WorkFirstCont(Player player)
        {
            ...
            NAPI.Task.Run(() =>
            {
                ...
                var nextCheck = WorkManager.rnd.Next(0, Checks2.Count - 1);
                while (nextCheck == player.GetData<int>("WORKCHECK"))
                    nextCheck = WorkManager.rnd.Next(0, Checks2.Count - 1);
        ...
        private static void EnterShapePutContAndTakeNew(ColShape shape, Player player)
        {
            try
            {
                ...
                if (Main.Players[player].WorkID != 12 || !player.GetData<bool>("ON_WORK") || shape.GetData<int>("NUMBER2") != player.GetData<int>("WORKCHECK")) return;
